...
def detect_objects_on_image(buf):
    ...
    """
    model = YOLO("best.pt")
    results = model.predict(Image.open(buf), conf=0.5, iou=0.5)
    result = results[0]
    output = []
    for box in result.boxes:
        ...
        bbox = [x1, y1, x2, y2, result.names[class_id], prob]
        output.append(bbox)

        # Write bbox to a text file
        with open('bbox_coordinates.txt', 'a') as f:
            f.write(str(bbox) + '\n')
